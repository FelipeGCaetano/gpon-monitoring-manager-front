...
// Este modal é uma cópia do create-container-modal, mas
// sem o campo "instanceId" e com um callback "onSave"
// em vez de uma chamada direta à API.
...
// Interface do Payload de Saída (o que o modal "pai" espera)
...
interface CreateInstanceContainerModalProps {
    ...
    onSave: (containerData: InstanceContainerData) => void // Callback para o modal pai
}
...
export function CreateInstanceContainerModal({
    ...
    // --- Carrega dados (Templates e Vars Globais) ---
    useEffect(() => {
        if (open) {
    ...
    // --- Popula EnvVars E VERIFICA VARS GLOBAIS ---
    useEffect(() => {
        if (!selectedImage) {
            setEnvVars([])
            return
        ...
        if (template && template.envDefinitions) {
            const newEnvs = template.envDefinitions.map((def: EnvDefinition) => {
                ...
                const value = isGlobal ? globalEnvMap.get(def.key)! : "";
                return {
    ...
    }, [selectedImage, imageTemplates, globalEnvs])

    // --- Handlers (exatamente como no modal de container) ---
    ...
    // --- Handler de Submissão (Modificado) ---
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (!selectedTemplate) {
            alert("Template de imagem selecionado não encontrado.");
            setIsSubmitting(false);
            return;
        }
        ...
        // Chama o Callback em vez da API
        onSave(payload)
        onOpenChange(false)
        setIsSubmitting(false)
    }
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col">
                <DialogHeader>
                    <DialogTitle>Adicionar Container à Instância</DialogTitle>
                    <DialogDescription>
                        Configure os detalhes do container que será criado com esta instância.
                    </DialogDescription>
                </DialogHeader>
                ...
                {isLoadingData ? (
                    ...
                    <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden">
                                        ...
                                        envVars.map((env) => (
                                            <div key={env.id} className="grid grid-cols-2 gap-3 items-center">
                                                ...
                                                <Input
                                                    ...
                                                    disabled={isSubmitting || env.isGlobal}
                        ...
                        <div className="mt-auto pt-6 border-t border-border">
                            <Button type="submit" className="w-full" disabled={isSubmitting || isLoadingData || !name || !selectedImage}>
                                {isSubmitting ? (
